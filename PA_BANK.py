...
def admin_create_account():
    try:
        ...
        pin = pin.replace(" ", "").strip()  # XMenghapus spasi dari PIN pakai replace
        ...
        if role == "1":
                while True:
                    ...
                    if opsi == "1":
                        ...
                        sorted_accounts = sorted(rekenings.items(), key=lambda x: x[0])  
                        merge_sort(sorted_accounts)  
